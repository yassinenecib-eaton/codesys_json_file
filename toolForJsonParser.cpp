// toolForJsonParser.cpp : Defines the entry point for the application.
//

#include "toolForJsonParser.h"
...
unsigned int searchSequence(std::vector <std::string> a_vector, std::vector<int> listTypeInteger, std::ofstream& a_Jsonfile)
{
    ...
    //Resize the vector because we use i+2 and it make an overflow 
    a_vectorSorted.resize(a_vectorSorted.size()+2);

    std::cout << "size de a_vector: " << a_vector.size() << std::endl;

     //Search this squence through the vector
    for (int i = 0; i < a_vectorSorted.size()-2; i++)
    {
...
int checkData(string a_string)
{
    std::size_t found = a_string.find("BI");
    if (found != std::string::npos)
    {
        //std::cout << "BI type found " << found << '\n';
        return 0;
    }

    found = a_string.find("BO");
    if (found != std::string::npos)
    {
        //std::cout << "BO type found " << found << '\n';
        return 1;
    }
...
int main()
{
    ...
    if (myfile.is_open())
    {
        ...
        myJsonfile.open("resultFile2.json");

        //Display sequence
        while (std::getline(myfile, line))
        {
            std::vector<std::string> sequenceVector = split(line, '\t');
            string tmp = sequenceVector[0];
            if (checkData(tmp) == -1)
            {
        ...
        while (std::getline(myfile, line))
        {
            ...
            else
            {
                ...
                else if (word.size() > 2)
                {
                    ...
                    std::cout << "  \"addr\":" + word[l] + "," << std::endl;
                    myJsonfile << "  \"addr\":" + word[l] + "," << std::endl;
                    std::cout << "  \"value\":" + word[l - 1] << std::endl;
                    myJsonfile << "  \"value\":" + word[l - 1] << std::endl;
                }
